...
CTE.languageCodeFromExternalAPI = function() {
	...
	switch (userLanguage) {
		...
		case "brazilian": languageCode = "ptbr"; break;
...
CTE.includeSpaceInItemNameDisplay = function() {
	var languagesWithSpace = [ "EN", "FR", "ES", "IT", "DE", "PTBR" ];
...
// Return whether the current language should have three-line textboxes.
CTE.isThreeLineLanguage = function() {
	var threeLineLanguages = [ ];
	for (var i = 0; i < threeLineLanguages.length; i++) {
		if (ConfigManager.isLanguage(threeLineLanguages[i])) {
			return true;
		}
	}
	return false;
...
// Change font size and line positioning to fit three lines instead of two in certain languages.
Window_Message.prototype.standardFontSize = function() {
	if (CTE.isThreeLineLanguage()) return 24;
    return 28;
};

Window_Message.prototype.calcTextHeight = function(textState, all) {
	var threeLines = CTE.isThreeLineLanguage();
	
	var lastFontSize = this.contents.fontSize;
	var textHeight = !threeLines? 2 : 6;
	...
	for (var i = 0; i < maxLines; i++) {
		...
		textHeight += maxFontSize + (!threeLines? 14 : 5);
	}
...
// Whitelist certain characters as hankaku (half width) so they don't have extra space afterward.
Window_Base.prototype.processNormalCharacterruby = function(textState) {
	var c = "";
	var m;
	var containsTH = false;
	do {
		c += textState.text[textState.index++];
		m = textState.text[textState.index];
		if (Window_Base.TH_MARK_LIST.contains(m)) {
			containsTH = true;
		}
	} while (Window_Base.TH_MARK_LIST.contains(m));
	
	if (containsTH || ConfigManager.isLanguage("TH")) {
		var w = this.textWidth(c);
		this.contents.drawText(c, textState.x, textState.y, w * 2, textState.height);
		textState.x += w;
	}
	else {
		if (c.match(hankaku) || c.match(/[áàâäåÁÀÂÃÄÅéèêëÉÈÊËíìîïÍÌÎÏñÑóòôöõøÓÒÔÖÕØúùûüÚÙÛÜýÿÝçÇæœß–«»’“”]/)) {//半角の場合
...
Game_Interpreter.prototype.setupPopupMessage = function(args) {
	...
	args[5] = CTE.getString("PopupText_" + args[5].replace("\\_", ""), args[5]);
	args[5] = CTE.getString("PopupText_" + args[1] + "Only_" + args[5].replace("\\_", ""), args[5]);
