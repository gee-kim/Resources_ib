...
(function() {
    ...
    Sprite_FtPopupMessage.prototype.setupDynamicText = function(text) {
        ...
        // Edit by vgperson to handle Thai marks and offset ones that are weirdly placed in font
        if (text) {
            if (!ConfigManager.isLanguage("TH")) {
                bitmap.drawText(text, 0, 0, bitmap.width, bitmap.height, 'center');
            }
            else {
                var mark = Window_Base.TH_MARK_LIST.contains(text);
				var w = this._italic ? size * 1.5 : size;
				if (!mark) {
					bitmap.drawText(text, 0, 0, bitmap.width, bitmap.height, 'center');
				}
				else {
					var offset = Window_Base.TH_MARK_OFFSET.contains(text);
					var under = Window_Base.TH_MARK_UNDER.contains(text);
					bitmap.drawText(text, 0, offset? ((size / 8) * (under? -1 : 1)) : 0, w * 1.5, bitmap.height, 'center');
				}
			}
        }
        return bitmap;
